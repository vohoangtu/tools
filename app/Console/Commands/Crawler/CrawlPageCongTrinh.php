...
class CrawlPageCongTrinh extends AbstractCrawler
{

    protected $photos = [];

    protected $signature = 'crawl:page-coingtrinh';
    ...
    {
        $links = [
            "http://dahoacuongbachthinh.com/cong-trinh-du-an.html",
            "http://dahoacuongbachthinh.com/cong-trinh-du-an.html&p=2"
        ];
        ...
        foreach($links as $link){
            ...
            $_urls = [];
            foreach($crawler->filter("body a")->links() as $uri){
                $_urls[] = trim($uri->getNode()->getAttribute("href"));
            }
            $matches = array_filter($_urls, function ($value){
                return Str::contains($value, ".html") && (Str::contains($value, "cong-trinh-du-an/"));
            });

            foreach ($matches as $url){
                $url = "http://dahoacuongbachthinh.com/" . Str::replace('">', '', $url);
